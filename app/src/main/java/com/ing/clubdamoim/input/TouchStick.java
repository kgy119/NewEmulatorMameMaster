...
package com.ing.clubdamoim.input;
...
import com.ing.clubdamoim.helpers.PrefsHelper;
...
*/
    ...
    protected void calculateStickPosition(Point pt) {
        ...
        } else if ( (ways == 4 /*&& b*/ || !b) && mm.getPrefsHelper().getControllerType() == PrefsHelper.PREF_DIGITAL_STICK ) {
            if (mm.getInputHandler().getTouchController().getStick_state() == STICK_RIGHT
				|| mm.getInputHandler().getTouchController().getStick_state() == STICK_LEFT) {
                stickPos.left = Math.min(ptMax.x - stickWidth, Math.max(ptMin.x, pt.x - (stickWidth / 2)));
                stickPos.top = ptCenter.y - (stickHeight / 2);
            } else {
    ...
    public int handleMotion(MotionEvent event, int pad_data) {
        ...
        if (actionEvent == MotionEvent.ACTION_UP ||
                ...
                (mm.getPrefsHelper().isTouchLightgun() &&
    ...
    public void draw(Canvas canvas) {
        ...
        if (Emulator.isDebug()) {
            canvas.drawText("x=" + ptCur.x + " y=" + ptCur.y + " state=" + mm.getInputHandler().getTouchController().getStick_state() + " rx=" + rx + " ry=" + ry + " ang=" + ang + " mag=" + mag, 5, 50, Emulator.getDebugPaint());
        }
    }
